...
/** Open an output file (desktop-only) — degrades to a toast on the web. */
function openOutputFile(path: string) {
  toast(`Opening ${path} in its default app is available in the desktop app.`);
}

/**
 * The runs ledger — pipeline-stage executions backed by the reproducibility
 * index. Faceted (status / surface), searchable, and keyset-paginated with
 * infinite scroll, so it stays fast and calm from five runs to hundreds of
 * thousands.
 */
        ...
        {state !== "loading" && rows.length === 0 && !anyFilter && (
          <div className="mt-8 rounded-input border border-dashed border-border bg-surface px-4 py-8 text-center">
            ...
            <p className="mx-auto mt-1 max-w-sm text-xs text-muted">
              When the agent runs code (e.g. <span className="font-mono text-text">python s1_review.py</span>), each
...
/** Global Runs view (sidebar) — all runs across every review pipeline. */
        ...
        <header className="mb-4 flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-sm text-muted">
              Every pipeline execution across all reviews — command, code version, environment, hardware, and outputs.{" "}
              <span className="text-text/70">Reproduce</span> re-runs it and compares results.
            </p>
          </div>
        </header>
        <RunsView />
      ...
      {open && (
        <div className="ml-6 mb-1 space-y-3 border-l border-border-faint pl-4 pt-1 text-xs">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {r.env?.packages && (
              <Chip icon={<Package size={11} />}>{r.env.packages.count} packages</Chip>
            )}
            {r.remoteHardware && (
