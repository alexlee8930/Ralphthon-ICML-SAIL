/**
 * Extract a structured reviewer result the agent emits as a ```review fenced
 * JSON block. Ported verbatim from Open Science Desktop (MIT); the `@ai4s/shared`
 * finding/review types it relied on are provided by `@/api/types` on the web.
 */
import type { FindingLevel, ReviewCheck, ReviewerBlock } from "@/lib/mock";
