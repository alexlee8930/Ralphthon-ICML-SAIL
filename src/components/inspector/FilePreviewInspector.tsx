...
import type { FilePreviewInspector as FilePreviewInspectorT, FileRoot } from "@/lib/artifacts";
...
} from "@/lib/artifactFileInspector";
import { parseTableFile } from "@/lib/csvInspector";
import { CodeViewer } from "@/components/code-viewer/CodeViewer";
import { MarkdownViewer } from "@/components/markdown-viewer/MarkdownViewer";
import { ProvenancePanel } from "./ProvenancePanel";
import { TablePreview } from "./TablePreview";
import { TableChart } from "./TableChart";
import { canChart } from "@/lib/tableChartInspector";
import {
  AnomalyMapView,
  BandView,
  DocxView,
...
/** Format a number for display (reference `i18n/format`, English locale). */
 ...
 * pdf / image / html — rendered from a real URL by the browser's NATIVE
 * viewers via <iframe>/<img>; csv/tsv — parsed to a table; everything else —
 * code/text from the inline content a writing tool carried.
 */
...
function Body({
  ...
  if (kind === "pdf") {
    // The browser's native PDF viewer renders the served URL.
    return url ? (
  ...
  if (kind === "video") {
    // A real URL streams straight into the native <video> element.
    return url ? (
      <div className="flex justify-center p-4">
        <video
          src={url}
...
function TableView({ table }: { table: import("@/lib/csvInspector").ParsedTable }) {
  ...
  return (
    <div className="p-4">
      <div className="rounded-card border border-border bg-surface p-4 text-sm text-muted">
        ...
        <p className="mb-3">
          Previews are capped so a large file can't freeze the app. Inspect it without loading — the
          large-file probe reads its schema, shape, a sample, and key numbers by streaming, never
          loading the whole file — or open it in your system app.
        </p>
...
function LargeFilePointerPanel({ p }: { p: LargeFilePointer }) {
  ...
  if (p.read_length)
    rows.push([
      "Read length",
      `${p.read_length.min}–${p.read_length.max} (mean ${p.read_length.mean})`,
    ]);
