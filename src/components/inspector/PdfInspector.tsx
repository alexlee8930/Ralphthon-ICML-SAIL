...
import type { PdfInspector as PdfInspectorT } from "@/lib/artifacts";
...
export function PdfInspector({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {data.url ? (
        // A real PDF: the browser's native viewer renders the URL directly
        // (same presentation as the file-preview PDF branch).
        <div className="min-h-0 flex-1 overflow-auto bg-surface-2">
          <iframe title="PDF preview" src={data.url} className="h-full min-h-[480px] w-full" />
        </div>
      ) : (
