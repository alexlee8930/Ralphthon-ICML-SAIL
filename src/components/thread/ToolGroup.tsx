...
import type { ThreadBlock, ToolCallBlock } from "./blocks-thread";
...
/** Render a unified diff with added/removed/hunk lines colored — private copy
 *  of the reference `@/components/code-viewer/DiffView` (same classes). */
function DiffViewInline({ diff, className }: { diff: string; className?: string }) {
  return (
    <div
      className={cn(
...
function DiffDetail({ diff }: { diff: string }) {
  return <DiffViewInline diff={diff} className="ml-7 mb-1 mt-0.5 max-h-64 overflow-y-auto" />;
}
  ...
  return (
    <div>
      ...
      >
        ...
        {failed > 0 && (
          <span className="shrink-0 text-error">· {failed} failed</span>
        )}
      </button>
      <Collapse open={open}>
