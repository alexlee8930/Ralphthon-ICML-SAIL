...
// Agent-request event shapes — ported from the desktop `@ai4s/sdk` (which does
// not exist on the web) and inlined so this surface stands alone. The Ralph web
// build has no blocking agent questions/permissions, so nothing wires these in
// today; the component is kept faithful for when the API grows them.
  ...
  return (
    <div className="rounded-card border border-accent/40 bg-surface shadow-card">
      <header className="border-b border-border px-4 py-2.5">
        ...
        {origin && (
          <div className="mt-0.5 pl-6 text-xs text-muted">Asked by {origin}</div>
        )}
      </header>
