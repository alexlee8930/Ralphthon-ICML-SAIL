...
import type { FindingLevel, ReviewCheck, ReviewerBlock } from "./blocks-thread";
import { cn } from "@/lib/cn";

const BADGE: Record<FindingLevel, { className: string; label: string }> = {
  warn: { className: "bg-warn/15 text-warn ring-warn/30", label: "Warn" },
  ok: { className: "bg-ok/15 text-ok ring-ok/30", label: "OK" },
  error: { className: "bg-error/15 text-error ring-error/30", label: "Error" },
};
    ...
    <div className="rounded-card border border-border bg-surface shadow-card">
      ...
      >
        ...
        <span className="text-sm text-muted">
          {visible.length === 1 ? "· 1 finding" : `· ${visible.length} findings`}
