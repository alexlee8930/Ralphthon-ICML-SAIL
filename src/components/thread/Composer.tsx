...
/** The two approval modes the composer can switch between (Codex-style). Ported
 *  from the desktop `@/lib/tauri` type — inlined so the web build stands alone. */
export type ApprovalMode = "approve" | "full";

// Desktop-only workspace file helpers. The web build has no native filesystem,
// so `canAttach` is always false and these never actually run — kept as no-op
// stubs so the ported code stays byte-identical to the reference.
async function addFilesToWorkspace(): Promise<string[]> {
  return [];
}
async function addTextToWorkspace(name: string, _text: string): Promise<string> {
  return name;
}
 ...
 *  (label/description) is resolved at render time — see `approvalCopy`. */
 ...
 * Ralph session passes one to submit prompts to the agent. Attached workspace
 * files show as removable chips above the input, not as prompt text.
 *
...
export function Composer({
  ...
  const { composerDraft, setComposerDraft } = useUiStore();
  ...
  const onPaste = (e: ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    void (async () => {
      try {
        ...
        toast.error(
          `Could not save paste: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    })();
  };
  ...
  const addFiles = async () => {
    ...
    try {
      ...
      toast.error(
        `Could not add files: ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setAdding(false);
    }
