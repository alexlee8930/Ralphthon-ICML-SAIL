...
import type { ToolCallBlock, ToolCallStatus } from "./blocks-thread";
import { cn } from "@/lib/cn";

// Icon + tone per status, with the English aria-label alongside (the reference
// looked these up from the session locale at render time).
export const STATUS: Record<ToolCallStatus, { icon: React.ReactNode; className: string; label: string }> = {
  pending: { icon: <Clock size={13} />, className: "text-muted", label: "Pending" },
  running: { icon: <Loader2 size={13} className="animate-spin" />, className: "text-accent", label: "Running" },
  "waiting-approval": { icon: <ShieldQuestion size={14} />, className: "text-warn", label: "Waiting" },
  success: { icon: <Check size={13} />, className: "text-ok", label: "Success" },
  warning: { icon: <AlertTriangle size={14} />, className: "text-warn", label: "Warning" },
  failed: { icon: <X size={14} />, className: "text-error", label: "Failed" },
};
