...
1. `/review`에서 제목+본문 제출 → AgentWorkingCard가 step/thinking 이벤트를 스트리밍
   → 리뷰 3건 도착. **이 시점에 점수가 어디에도 없어야 한다.**
...
| 1 | POST /api/loop/papers | submit.json | cycles[0].reviews 3건(rating 1-10)·comments·thread에 리뷰 메시지·**score 없음** |
...
| 4 | POST …/revision-apply | revision-apply.json | draftManuscript + revisionNote 생성, pendingRevision 소거 |
| 5 | POST …/finalize | finalize.json | metaReview·score{score,gradeTier,attributions,layers}·decision·deficiency·decisionPost |
| 6 | POST …/resubmit | resubmit.json | cycles 길이+1, 새 사이클 manuscript==이전 draft, thread 빈 배열 아님(리뷰 도착) |

## C. 불변식 (모드 무관)

- score/decision/deficiency는 finalize 이후에만 존재.
- reviews[].reviewer는 "Reviewer 1..3" — 익명 핸들 없음.
- placeholder 원고(한 줄 인사말 등) → rating 1 + 헝크 0 (그라운딩).
