# 60 · 백엔드 어댑터 (FastAPI · 4-헤드 오케스트레이션)

의존: contracts/20 (HTTP 계약)

산출 파일:

- `serve/sail_adapter.py`
- `serve/requirements.txt`

---

단일 파일 FastAPI 서버 — contract v2의 서버측 전부. SPA 정적 서빙 겸용.

아키텍처 (4-헤드 파이프라인):
1. **리뷰어 헤드** (현재 Claude 대행, 서빙되면 astage-reviewer로 교체):
   3 리뷰어 병렬(ThreadPoolExecutor+emitter 전파) → 판단(요약·평점·이슈·
   facet 점수·strengths) → **확장 레이어**가 ICML 분량(350-550단어) body 생성.
2. **메타+점수 헤드** (VESSL Qwen3-8B LoRA, POST {VESSL_META_URL}/meta-review):
   서버가 프롬프트를 소유하므로 클라이언트는 reviews 문자열 배열(각각
   "Rating: N: {RATING_ANCHORS[N]} | Confidence: …" 헤더 + body), abstract,
   discussion [{who,text}] 만 보낸다. 응답 p_accept → 점수 캘리브레이션:
   p^0.25×100 clamp(1,99) 후 앵커 블렌드 0.6·cal + 0.4·(평균평점×10).
3. **설명 헤드**: leave-one-out 스타일 attribution + deficiency 리포트
   (다음 밴드 목표·feature별 why/action).
4. **수정 에이전트**: grounding 엄수(실험 날조 금지·placeholder는 헝크 0)
   + ```svg 펜스로 실제 figure 생성. 앵커는 공백-불감 매칭 폴백.

운영 요점:
- ANTHROPIC_API_KEY 없으면 모든 헤드가 결정적 폴백으로 강등(LIVE=False).
- Claude 호출: claude-opus-4-8, adaptive thinking(summarized) 스트리밍,
  structured outputs(output_config json_schema). thinking 델타는 잡 이벤트로.
- 잡: _jobs 레지스트리 + contextvar emitter, 데몬 스레드 (VALID_OPS 참조).
- 상태: SAIL_STATE_PATH JSON 파일 (v1 상태 자동 폐기).
- **리센시 보정**: TOPIC_MATURITY_PATH(topic_maturity.json, backend/61)를
  로드해 성숙-토픽(급감 AND 최근<0.5%)만 실측 수치 노트로 abstract에 주입.
  상승 토픽 동반 시 억제. Claude 판단 0% — 코퍼스 측정 100%.
- 실행: `python sail_adapter.py` (PORT 8100, WEB_DIST로 SPA 서빙).
...
### 파일: `requirements.txt` (6줄) — **verbatim, 글자 그대로 사용**
