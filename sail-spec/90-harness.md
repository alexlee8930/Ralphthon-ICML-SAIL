...
## 4.5 동반 하네스 — ac-competition-kit (juneyoon-sweetspot/ac-competition-kit)

VESSL 학습·서빙 트랙(training/85)의 원본 하네스. sail-spec이 "제품 재현"을, kit이
"모델 재현"을 소유한다 — GPU 창에서는 kit을 clone해 함께 쓴다. 거기서 가져와 이
하네스에도 적용하는 규율 3가지:

1. **STATE.md 규율**: 잡 발사 즉시 `[~] 단계 job-ID HH:MM`, 완료 즉시 `[x]`+수치,
   판단마다 한 줄 사유. "STATE에 없는 진행상황은 존재하지 않는 것과 같다."
   → 멀티에이전트 웨이브(§2)에서도 오케스트레이터가 동일 규율로 진행 로그를 남길 것.
2. **검증본 복붙 원칙**: 커맨드는 kit vessl/commands.sh·이 번들의 런북에 있는 검증본을
   복붙한다 — 창작 금지. 새 에러만 새로 디버깅한다 (지뢰표 먼저 대조).
3. **무인 운행**: 사람에게 묻지 않는다. 실패 시 ①지뢰표 대조→처방 ②일시 오류 20초
   재시도 ③폴백 경로 전환, 같은 잡 3회 실패면 스킵하고 기록.

백엔드 배포 대안: `bash kit/backend/deploy_gce.sh <VM이름>` — VM 생성→방화벽→어댑터→
systemd→스모크 원샷 (ops/80의 startup-script 방식과 동일 결과, 병행 배포 가능).
스모크: `bash kit/backend/smoke.sh http://<IP>:8100`.
