...
(Vite + React 18 + TS + Tailwind 3 + recoil/react-query SPA · FastAPI 단일파일 백엔드 ·
...
10. **시크릿**: 평문 키는 `sail-spec/SECRETS.local.md`(gitignore 대상)에 있다 — git clone으로
    번들을 받았다면 이 파일이 빠져 있으니 **반드시 별도 확보** 후 시작할 것 (없으면 LIVE·
    G3·G4 전부 불가). 코드에는 하드코딩하지 않고 환경변수/메타데이터로 주입한다.
...
- 프론트 단위가 끝날 때마다 npx tsc -b로 타입 오류를 그 자리에서 잡고 다음 단위로.
  단, ui/50번대는 상호 참조가 있으므로 55→52→53→50→51→54 순으로 구현하거나
  50번대 전체 완료 후 일괄 검증한다.
...
- 키 없이(폴백): fastapi TestClient로 golden/flows.md B표 순서 그대로 —
  POST papers(multipart) → reply → revision-draft → revision-apply → finalize →
  resubmit. finalize 후 score·metaReview·deficiency·decisionPost 존재,
  resubmit 후 cycles 길이 +1. 상태 파일 재기동 후 유지.
