# 30 · reviewLoop API (타입 계약 + mock 시뮬레이션 + live 클라이언트)
...
제품의 심장. 한 파일이 ① 단일 타입 계약 ② mock 시뮬레이터(결정적)
③ live HTTP 클라이언트 ④ 비동기 잡(runMockJob / startOp) 모두를 소유한다.

동작 요점 (구현자가 놓치기 쉬운 것):
- `BASE = import.meta.env.VITE_RALPH_API_URL` 유무로 모든 loopApi 함수가
  mock/live 분기한다. UI는 이를 모른다.
- 점수는 `mockFinalize`/서버 finalize에서만 생긴다. SELECT_THRESHOLD = 88.
- mock 잡은 단계 이벤트(step/thinking)를 지연 스케줄로 흘려 실제 에이전트
  스트리밍처럼 보이게 한다 — 이벤트 문구도 아래 verbatim 그대로.
- resubmit 은 draftManuscript 를 새 사이클 manuscript로 옮기고 스레드를 비운다.
- FALLBACK_REVIEWS·CYCLE_SCORES·tierFor·layersFor·attributionsFor 는 데모
  결정성의 원천이므로 숫자 하나도 바꾸지 말 것.


---

### 파일: `src/api/reviewLoop.ts` (709줄) — **verbatim, 글자 그대로 사용**
