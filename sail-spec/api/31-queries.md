...
- `src/api/reviewLoopQueries.ts`

---

모든 화면은 이 훅으로만 데이터에 접근한다 (loopApi 직접 호출은
ReviewLoopPage의 startSubmit/startOp 진입점뿐). `useAgentJob` 은 잡이
running 인 동안 700ms 간격 refetch — 에이전트 스트리밍 UI의 심장박동.


---

### 파일: `src/api/reviewLoopQueries.ts` (86줄) — **verbatim, 글자 그대로 사용**
