...
- `src/components/settings/RemoteComputeCard.tsx`
- `src/components/settings/ModalCard.tsx`
- `src/components/settings/DataFlowCard.tsx`

---

백엔드 연결 상태·모델 구성·데이터 흐름을 보여주는 정적/반정적 카드들.


---

### 파일: `src/app/routes/SettingsPage.tsx` (286줄) — **verbatim, 글자 그대로 사용**
...
### 파일: `src/components/settings/RemoteComputeCard.tsx` (26줄) — **verbatim, 글자 그대로 사용**
...
### 파일: `src/components/settings/ModalCard.tsx` (24줄) — **verbatim, 글자 그대로 사용**
...
### 파일: `src/components/settings/DataFlowCard.tsx` (47줄) — **verbatim, 글자 그대로 사용**

````tsx
import { HardDrive, Send } from "lucide-react";

/**
 * Plain-language disclosure of what stays local vs. what is sent to the review
 * model provider. Every statement here must stay true to the actual
  ...
  return (
    <section className="mt-5 rounded-card border border-border bg-surface shadow-card">
      <header className="border-b border-border px-5 py-3">
        ...
        <p className="mt-0.5 text-xs text-muted">What stays on this machine, and exactly what leaves it.</p>
      </header>
      <div className="grid gap-5 px-5 py-4 sm:grid-cols-2">
        <div>
          ...
          <ul className="mt-2 list-disc space-y-1.5 pl-4 text-[13px] leading-relaxed text-muted">
            <li>
              Your papers, reviews, and raw uploads
              {workspace && <span className="font-mono text-xs"> ({workspace})</span>}.
            </li>
            <li>Pipeline execution — the scoring and analysis code runs locally; documents are processed here, never uploaded in bulk.</li>
            <li>Review history and provenance records, in the app's private data folder.</li>
        ...
        <div>
          ...
          <ul className="mt-2 list-disc space-y-1.5 pl-4 text-[13px] leading-relaxed text-muted">
            <li>The paper text and your messages, plus the content the agent reads to review the paper you asked about.</li>
            <li>Nothing is sent in the background — data leaves only during a review turn.</li>
            <li>What the provider retains is governed by its own data policy.</li>
          </ul>
          <p className="mt-2 text-xs text-muted">Skills and MCP servers you add may make their own network calls — review them before installing.</p>
        </div>
      </div>
    </section>
  );
