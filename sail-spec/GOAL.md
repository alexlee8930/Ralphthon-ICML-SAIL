...
0. GPU 학습 발사 (맨 먼저 — 원격에서 혼자 돌므로 다른 작업과 병행된다):
   training/85 §1.5 단축 경로, **3시간 컷 구성**. vesslctl billing show로 크레딧
   규칙 확인 후, vendor/ac-competition-kit/BACKLOG.md P1 커맨드에서 리소스만 4×로
   상향해(-r resourcespec-ch100x4, --nproc_per_node 4 — ddp4-smoke로 검증된 구성)
   실행 (astage-v2, /data/sft_astage_v2, ~2.2h) → job-ID를 STATE.md에 기록하고
   즉시 1번으로 넘어간다. T+2:15 평가 체인 → T+3:00 게이트 판정이 데드라인이다.
   감시는 로그가 아니라 job state·볼륨 산출물로 (로그 60-75분 지연, 침묵≠사망).
   잔액 부족 시에만 2×H100으로 강등(3h 초과를 STATE에 명시)하고 충전 요청 후 계속.
...
2. P1 (최우선, 3시간 컷): kit BACKLOG P1 커맨드에서 리소스만 4×로 상향 —
   -r resourcespec-ch100x4, torchrun --nproc_per_node 4 (ddp4-smoke 검증 구성),
   데이터 /data/sft_astage_v2, ~2.2h. T+3:00 게이트 판정이 데드라인. 발사 즉시
   STATE.md에 job-ID·시각 기록. 감시는 로그가 아니라 job state·볼륨 산출물로
   (로그 60-75분 지연, 침묵≠사망, 타임아웃 전 임의 kill 금지). 잔액 부족 시에만
   2×H100 강등(3h 초과 명시).
